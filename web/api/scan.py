...
# Import scanner components from the published package
from src.scanner.core import PromptScanner
from src.utils.repo_fetch import parse_github_url, fetch_github_repo_to_dir

# Reuse the CLI serializer for consistent payloads
try:
    from cli import _serialize_scan_results_to_json  # type: ignore
except Exception:
    _serialize_scan_results_to_json = None  # Fallback to local serializer if missing
...
def scan_handler():
    ...
    try:
        ...
        # Optional second-stage filter by min_confidence to mirror CLI behavior
        if min_confidence > 0:
            filtered = []
